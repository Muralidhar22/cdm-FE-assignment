...
const NavLink = ({ selectedTab, tabName, href }: NavLinkPropsType) => {
    return(
        ...
        <Link shallow className={`cursor-pointer font-semibold pr-20 py-3 ${selectedTab === tabName ? "text-black" : "text-zinc-400"}`} href={`/edit?details=${href}`}>{tabName ? tabName.charAt(0).toUpperCase() + tabName?.slice(1): null}</Link>
        </span>
    )
}
