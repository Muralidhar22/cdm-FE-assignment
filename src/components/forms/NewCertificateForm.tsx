...
const NewCertificateForm = () => {
    ...
    const onChangeSelectHandler = (e: React.ChangeEvent<HTMLSelectElement>, type: "techStack" | "certType") => {
        if(type === "techStack") {
            const value = findTechStackValue(e.target.value)
            value && setFormData(prev => ({ ...prev, technology: { ...value } }))
        } else if (type === "certType") {
            setFormData(prev => ({ ...prev, certType: e.target.value as CertType }))
        }
    }
    ...
    const onSubmitHandler = (e: React.FormEvent) => {
        e.preventDefault()
        if(formData) {
            setPortfolioData((prev) => ({ ...prev, certificates: [...prev.certificates, { ...formData}] }))
        }
    ...
    return (
        ...
        <select onChange={(e) => onChangeSelectHandler(e,"certType")} value={formData.certType} className="w-full px-3 py-3.5 rounded-lg border-2 border-zinc-100 focus:border-primary-600 cursor-pointer" id="techStack">
            <option value="" disabled>--choose option--</option>
               ...
               <>
               <label htmlFor="techStack" className="block font-semibold text-sm" >Select tech stack for playground:</label>
        <select onChange={(e) => onChangeSelectHandler(e,"techStack")} name="techStack" value={formData.technology?.id} className="w-full px-3 py-3.5 rounded-lg border-2 border-zinc-100 focus:border-primary-600 cursor-pointer" id="techStack">
            <option value="" disabled>--choose option--</option>
