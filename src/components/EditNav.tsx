...
type EditNavPropsType = {
    selectedTab: editNavLinks
    tabName: editNavLinks
    href: string
}

const EditNav = ({ selectedTab, tabName, href }: EditNavPropsType) => {
    return(
        ...
        <Link shallow className={`cursor-pointer font-semibold pr-14 ${selectedTab === tabName ? "text-black" : "text-zinc-400"}`} href={`/edit?details=${href}`}>{tabName.charAt(0).toUpperCase() + tabName.slice(1)}</Link>
        </span>
    )
}
